                        ...
                        <table class="table table-hover table-bordered border-dark">
                            ...
                            <tbody class="text-center">
                                <tr v-for="(skills, index) in skillsList" :key="index">
                                    ...
                                    <td width="20%">{{ this.getCalculateYearMonth(skills.date_start) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
import skills from "../plugins/skills/skills.json"
export default {
    name: 'Skills',
    data: () => ({
        skillsList: skills.skills_data
    }),
    methods: {
        getImagePathAuto: function(payload) {
            return require("@/assets/image/skills/" + payload.toLowerCase() + ".png");
        },
